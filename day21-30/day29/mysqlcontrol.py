...
# 链接mysql服务器
    ...
    def insert_data(self,table_name, column_value_dict:dict, commit = False):
        ...
        if commit:
            self.__db.commit()
    # 删除记录
    def delete_record(self, table_name, where:list = None,
                      order_by:dict = None, limit = None,
        ...
        if commit:
            self.__db.commit()
    # 修改一个记录
    def update_record(self, table_name, content_set:dict,
                      where:list = None,
                      order_by:dict = None, limit = None,
                      relation_AND = True, commit = False):
        sql = """
        ...
        if commit:
            self.__db.commit()
    # 筛选记录
    def select_records(self, table_name, column_list:list = None,
                       where:list = None,
                      order_by:dict = None, limit = None,
                      relation_AND = True, commit = False):
        sql = """
    ...
    def __join_where_tuples(self, where:list, relation_AND):
        # where == (("name","%zhansan%", "like"), ("age":15))
        where_list = []
        for i in where:
        ...
        if relation_AND:
            ...
            relation = "OR"
        return " WHERE %s " % (" %s " % relation).join(where_list).replace("%","%%", 1000)
